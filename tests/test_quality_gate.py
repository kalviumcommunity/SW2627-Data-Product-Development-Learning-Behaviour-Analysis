...
def test_invalid_source_dataset_blocks_pipeline():
    broken = source_data()
...
def test_invalid_completion_range_blocks_pipeline():
    broken = student_course_data()
    broken.loc[0, "completion_pct"] = 101

    with pytest.raises(
        ValueError,
        match="expected range 0-100",
    ):
        validate_pipeline_output(
            {"completion": source_data()},
            broken,
...
def test_write_quality_report_rejects_non_dataframe(tmp_path: Path):
    with pytest.raises(
        TypeError,
